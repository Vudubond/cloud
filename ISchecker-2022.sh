...
# Copyright 2025, Vu3u <vu3u@gmail.com>
# Original Script made by  Rostyslav Yatsyshyn <rossardy@gmail.com>
...
# input:  list of versions; mode;
#if [ "$3" != 'CP_ALARM' ] ; then \
   for i in `echo "$1"| awk '{print $2}'|sort -u` ;
       do echo -n 'Store version:   ' `echo $1 | grep $i | awk '{print $3}' |uniq` '  |  Source RPM: '  $i '  |  On HV(s) and BS(s):  ';
             ip_sort_fun "$(echo "$1"|grep "$i"|awk '{print $1}')" '4';
             echo ;
   done;
#else i=$(echo $is_ver_short|awk '{print $2}'|uniq|grep -vE "^$")
#     echo -n 'Store version:   ' $i '  |  On HV(s) and Bs(s):  ';
#     echo;
#fi ;
   echo -n 'Store version:   ' $onapp_store_ver_dig '  |  CP version: ' $onapp_cp_ver ; unset i;
}
...
# function calculate info from HV
  ...
  is_ver=$(echo -e "$is_ver""\n"$hv "$(echo "$hv_stats"|grep 'is_ver '|sed 's/is_ver//g')" "$(echo "$hv_stats"|grep is_ver_short|sed 's/is_ver_short//g')");                                      # echo $is_ver; # get is ver
  is_con_ver=$(echo -e "$is_con_ver""\n"$hv "$(echo "$hv_stats"|grep is_con_ver|sed 's/is_con_ver//g')" "$(echo "$hv_stats"|grep is_ver_short|sed 's/is_ver_short//g')");                                     # take amount of storage controllers
...
if [ "$(ifconfig  2&>/dev/null| grep -i mgt | wc -l)" -ne 0 ]
then ip_addr=$(ifconfig mgt|grep Mask|awk {'print $2'}|sed 's/addr://')
...
is_ver_alarm=$(echo "$is_ver"|awk '{print $2}'|uniq|grep -vE "^$"|grep [a-z0-9]|wc -l)                                                                  # alarm =  IS ver mismatch all hvs
is_con_ver_alarm=$(echo "$is_con_ver"|awk '{print $2}'|uniq|grep -vE "^$"|grep [a-z0-9]|wc -l)
...
if [ "$is_ver_alarm" -gt 1 -a "$is_con_ver_alarm" -gt 1 -a  "$is_ver_short_alarm" -gt 1 ] ; then \
  [ "$is_ver_alarm" == 1 -a "$is_con_ver_alarm" == 1 -a  "$is_ver_short_alarm" == 1 ] && [ "$(echo "$is_ver_short"|awk '{print $2}'|uniq|grep -vE "^$" | sed 's/-[0-9]*$//g')" != "$(echo $onapp_cp_ver | sed 's/-[0-9]*$//g' | sed 's/^[a-z]*-[a-z]*-//g')" ] && red '**Mismatch onapp-store version:**' && \
    ...
    red "**found out mismatch IS versions in 'controllerversion|package-version'**";
       ...
       elif [ "$is_ver_alarm" -gt 1 ] ;                                                                                                       # Is ver mismatch all hvs report
       then red "**found out mismatch IS versions in 'cat /onappstore/package-version.txt'**   <-- package-version";
            advance_print_fun "$(rpm_sort_fun "$is_ver"|grep -vE "^$"|sort -k 4 -r)" '2' ;
